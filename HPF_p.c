...
for(run = 0 ; run < 5 ; run ++)
{
	...
	void recordNest(job j, record* tot, record* lvl)
	{
			...
			/*CHECK ERROR DEBUGGING*/
//			if(wait >100 || turn > 100 || response > 100)
//			{
//
//			}

//			if(wait <0 || turn < 0 || response < 0)
//			{
//
//			}
	...
	/*Put paused jobs here*/
	void insertFront(queue* q, job j)
	{
		q->count +=1;
		q->front = q->front - 1;
		q->array[q->front] = j;
    ...
    printf("\nHighest priority first (HPF) (preemptive) Run #%d\n", run+1);
	/* set job as CURRENT, begin work on a job*/
	void work(job j, int time)
	{
		currExist = 1;
		current = j;
		/*If restarting job: add time waited when sleeping*/
		if(current.started == 1)
		{
			printf("restarting  [ %c ] P: %d, T: %d \n", current.index, current.priority, time);
			current.waiting_T += (time - current.tempstop_T);
		}
		/*If first time waking job: initialize variables*/
		else
		{
	...
	/*pause a job; add it to front of its' priority queue*/
	void pause (job j, int time)
	{

		printf("pausing [ %c ] P: %d, T: %d  \n", j.index, j.priority, time);
		currExist = 0;
		j.tempstop_T = time;
		switch(j.priority)
			{
	...
	for(i = 0; i<100; i++)
	{
		...
		/*If CURRENT job exists : pick higher priority job to work on if possible*/
		if (currExist == 1)
		{
			if(hasQueue(lvl1) == 1 && current.priority > 1)
			{
				pause(current, i);
				work(dequeue(&lvl1), i); 	// i is timeslic
			}
			else if(hasQueue(lvl2) == 1 && current.priority > 2)
			{
				pause(current, i);
				work(dequeue(&lvl2), i);
			}
			else if(hasQueue(lvl3) == 1 && current.priority > 3)
			{
				pause(current, i);
				work(dequeue(&lvl3), i);

			}// if priority 4 then don't change.
		}
	...
	/*After 100 quanta, check for started (but not finished) jobs, complete and record stats*/
	int extra_wait = 0; // extra time waited past 100
	if(currExist == 1)
	{
		extra_wait += current.remain_T;
		recordR(current);
	}
	while(1)
	{
		/*work down priority to finish all started jobs*/
		if(peek(lvl1).started == 1)
		{
			current = dequeue(&lvl1);
				current.waiting_T += (100 - current.tempstop_T + extra_wait);
				extra_wait += current.service_T;
				recordR(current);
		}
		else if(peek(lvl2).started == 1)
		{
			current = dequeue(&lvl2);
				current.waiting_T += (100 - current.tempstop_T + extra_wait);
				extra_wait += current.service_T;
				recordR(current);
		}
		else if(peek(lvl3).started == 1)
		{
			current = dequeue(&lvl3);
				current.waiting_T += (100 - current.tempstop_T + extra_wait);
				extra_wait += current.service_T;
				recordR(current);
		}
		else if(peek(lvl4).started == 1)
		{
			current = dequeue(&lvl4);
				current.waiting_T += (100 - current.tempstop_T + extra_wait);
				extra_wait += current.service_T;
				recordR(current);
